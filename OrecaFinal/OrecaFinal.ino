...
void setup()
{
  ...
  Serial.print("L1_stat: ");
  Serial.print(L1_stat);
  Serial.print(" || L2_stat: ");
  Serial.println(L2_stat);
  ...
  // Send Devicetype & # of Restarts to Firebase
  FireFlag[1] = 1;
  FireFlag[2] = 1;
...
void loop()
{
  ...
  if (!(FireOrder == "0") && !(FireOrder == "connection lost")) {
    if (FireOrder == "RESET") {
      Serial.printf("Set RESET... %s\n", Firebase.setBool(fbdo, WiFi.macAddress() + "/RESET", RESETNumber) ? "ok" : fbdo.errorReason().c_str());
      FactoryReset();
    }
    if (FireOrder == "UPDATE") {
    ...
    //Action Operation
      ...
      Devices(devicetype, FireOrder, 1);
      Status = String(L1_stat) + "," + String(L2_stat);
...
void FireReport()
{
  ...
  if (FireFlag[0] == 1)
  {
    if (Firebase.setString(fbdo, WiFi.macAddress() + "/NodeStatus", Status)) {
      FireFlag[0] = 0;
      Serial.println("FireReporter ... OK");
    }
    else {
...
//--------------------------------------------------- Devices IO ------------------------------------------------
void Devices(char devicetype, String Order, bool FireOrTCP)
{
  yield();  // For ESP8266 to not dump
  ...
  if (devicetype == 'S') {               // ................................................. Roller Shutter
    ...
    if ((digitalRead(SW1) == HIGH && digitalRead(SW2) == HIGH) || Order == "STOP") // ------------------ Stop
    {
      //LED OFF
      digitalWrite(Anode , LOW);
    ...
    else if (digitalRead(SW1) == HIGH || Order == "UP") // ------------------------- UP
    {
      // NodeStatus = "UP";
      if (L2_stat == 1)
      {
    ...
    else if (digitalRead(SW2) == HIGH || Order == "DOWN") // -------------------- Down
    {
      //      NodeStatus = "DOWN";
      if (L1_stat == 1)
      {
  ...
  if (devicetype == 'L') {                         // ............................ 2 switches/plugs/loads
    if (digitalRead(SW1) == HIGH || Order == "OFF1" || Order == "ON1") // ----------- First switch
    {
      if (L1_stat == 0 || Order == "ON1")
      {
        //LED Yellow
        digitalWrite(Anode , LOW);
        digitalWrite(Cathod, HIGH);
    ...
    else if (digitalRead(SW2) == HIGH || Order == "OFF2" || Order == "ON2") // -------- Second switch
    {
      if (L2_stat == 0 || Order == "ON2")
      {
        //LED Yellow
        digitalWrite(Anode, LOW);
        digitalWrite(Cathod, HIGH);
